...
public class ChessMatch {
	...
	private ChessPiece king(Color color) {
		return getPieces(color).stream().filter(p -> p instanceof King).map(p -> (ChessPiece) p).findFirst()
	...
	public List<Piece> getOpponentPieces(Color color) {
		return piecesOnTheBoard.stream().filter(p -> ((ChessPiece) p).getColor() == opponent(color)).toList();
	}
